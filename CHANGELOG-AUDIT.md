...
### Phase-1 runtime verification (live production fetch) — 2026-07-07 (batch 4)
Fetched live production (`rkmfoundation.com`) via web_fetch. **VERIFIED live:** robots.txt correct (allow /, disallow /api /admin /prototype*, sitemap pointer) · unique per-page `<title>`/`meta-description`/canonical on `/`, `/shop`, `/donate-now` · `next/image` optimizer active in prod (`/_next/image?...`) · one H1/page · skip-link present · donate flow matches §8 (monthly-default, 2nd-tier "Most Chosen", fee-cover opt-in, named social proof, 95/5 breakdown, PAN/address optional, Razorpay UPI/cards). Two **fresh correctness findings** fixed below.

| # | Date | Finding (runtime-verified) | File | Change | Verification | Rollback |
|---|---|---|---|---|---|---|
| 31 | 2026-07-07 | **Self-red-team:** my batch-2 `Product` schema declared `availability: InStock` + priced Offer, but the live shop is **pre-launch** ("Launching Soon — Notify Me", "online checkout opens soon", orders via WhatsApp). Misleading structured data. | `app/shop/page.tsx`, `app/hi/shop/page.tsx` | `availability` → `https://schema.org/PreOrder` (EN+HI) to match reality | live copy confirms pre-launch; `tsc` clean | `git checkout` |
| 32 | 2026-07-07 | **Live evidence disproved an earlier assumption:** production `/shop` `og:title`="RKM Foundation" + generic `og:description` — every page shares the layout's hardcoded OG card (Next does NOT auto-derive when `layout.openGraph.title` is set). Real social/SEO gap. | `app/shop/page.tsx`, `app/hi/shop/page.tsx` | Added per-page `openGraph` (shop-specific title/description, EN+HI) | `tsc` clean; corrects the generic-card propagation | `git checkout` |
